...
int getIndex(ListRef L){
	...
	if( L->current != NULL){
		if( N->data == L->current->data){
			return count;
		}
		while( N != NULL){
			count = count + 1;
			if( N->next->data == L->current->data){
				return count;
			}
			N = N->next;
		}
...
void deleteCurrent(ListRef L){
	...
	if( L->current->data == L->front->data){
		N = L->front;
		L->front = N->next;
		freeNode(&N);
		L->current = NULL;
	}
	else if( L->current->data == L->back->data){
		N = L->back;
		L->back = N->previous;
		freeNode(&N);
		L->current = NULL;
